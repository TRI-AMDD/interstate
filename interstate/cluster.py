# modified from https://github.com/LLNL/graphite/blob/main/src/graphite/nn/utils/cluster.py
...
def _knn_graph(x: Tensor, k: int, loop: bool = False) -> Tensor:
    ...
    dist += torch.eye(x.size(0), device=x.device) * (1.0 - float(loop)) * 1e6
...
def radius_graph(
    x: Tensor, r: float, batch: Optional[Tensor] = None, loop: bool = False
) -> Tensor:
...
def periodic_radius_graph_v2(x: Tensor, r: float, box: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    cells = -torch.ones(
        num_cells, max_atoms_per_cell, dtype=torch.long, device=x.device
    )
    ...
    nbr_disp = torch.cartesian_prod(*torch.arange(-1, 2).expand(num_dims, 3)).to(
        x.device
    )
    ...
    ii, jj = torch.cartesian_prod(
        torch.arange(max_atoms_per_cell), torch.arange(max_atoms_per_cell)
    ).T.to(x.device)
