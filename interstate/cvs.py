# Modified from https://github.com/luigibonati/mlcolvar/blob/main/mlcolvar/cvs/cv.py
...
class BaseCV:
    ...
    def example_input_array(self):
        ...
        # return Data(
        #     pos = torch.tensor([[0.5,1,2], [2,2,2]], dtype=torch.get_default_dtype()), #!!!
        #     node_attr = torch.tensor([[1,0], [0,1]], dtype=torch.get_default_dtype()),
        #     node_input=torch.ones(2, 1),
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x, cv = self.forward_cv([x["pos"], x["batch"], x["node_attr"], x["node_input"]])
    ...
    def forward_cv(self, x: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Execute sequentially all the blocks in self.BLOCKS unless they are not initialized.
